...
## 版本 1.6.0：本地医疗 AI 与稳定性升级

- 新增 OpenMed 本地医疗 AI，可进行医学实体提取和隐私信息识别/脱敏。
- 新增轻量中文转英文与医学词典分析，普通 CPU 可快速完成演示。
- 修复部分业务请求误退出登录，会话默认有效期调整为 24 小时。
- 数据看板改为以数据库统计为准，避免 WebSocket 重连造成数字漂移。
- 修复患者风险分析的旧字段、空数据和急诊科统计问题。

## 可选：OpenMed 本地医疗 AI

启用后可在本机提取疾病、药物、基因等医学实体，并识别或脱敏个人隐私信息：

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\start-windows.ps1 -LocalAI
```

模型首次使用需要下载，后续可离线运行已缓存模型。OpenMed 不开放宿主机端口，输入原文默认不写数据库。当前官方 PII 模型不支持中文，完整说明见 [OPENMED.md](./OPENMED.md)。

医学实体提取默认采用轻量快速模式：中文先在本机翻译为英文，再结合医学词典识别，普通 CPU 通常一秒内返回。
